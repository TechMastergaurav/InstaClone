  ...
  return (
    ...
    <div className='my-8 w-full max-w-sm mx-auto'>
      ...
      <div>
        ...
        {console.log("User ID:", user?._id, "Post Author ID:", post?.author?._id, "Are they equal?", user?._id == post?.author?._id)}
        {user?._id == post?.author._id && (
          <span className="bg-gray-100 text-gray-700 rounded-full px-3 py-1 text-sm font-semibold ml-2 inline-block shadow-sm">
            Author
          </span>
        )}
      </div>
      ...
      {/* Only show if there is at least one comment */}
      {comment.length > 0 && (
        <span  onClick = {()=>{
            dispatch(setSelectedPost(post))
            setOpen(true)
